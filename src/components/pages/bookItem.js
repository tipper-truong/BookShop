...
class BookItem extends React.Component {
	...
	handleCart() {
		...
		if(this.props.cart.length > 0) {
			// Cart is not empty and adding same product, increase quantity
			...
			// if return -1, there are no items with the same ID
			if (cartIndex === -1) {
					...
					<Col xs={6} sm={8}>
						...
						<p>{(this.props.description.length > 50 && this.state.isClicked === false) ? 
							(this.props.description.substring(0, 50)):(this.props.description)}
								...
								{(this.state.isClicked === true && this.props.description !== null && this.props.description.length > 50) ? 
								('...Read Less'):('')}
								</button>
